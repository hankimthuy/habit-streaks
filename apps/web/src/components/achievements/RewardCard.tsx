...
export default function RewardCard({ rewards }: RewardCardProps) {
  const t = useTranslations("Achievements.rewards");
    ...
    <div className="px-4 mb-6">
      <div className="flex items-center justify-between mb-3 px-1">
        ...
        <span className="text-xs font-semibold text-slate-400">
          {t("unlocked", { unlocked: rewards.filter((r) => r.unlocked).length, total: rewards.length })}
        </span>
      </div>
      <div className="flex gap-3 overflow-x-auto pb-2 hide-scrollbar">
        {rewards.map((r) => {
          ...
          return (
            <div
              key={r.id}
              className={`flex-shrink-0 w-40 rounded-xl border p-3 relative overflow-hidden ${r.unlocked
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-2">
                  <div
                    className={`w-9 h-9 rounded-lg flex items-center justify-center ${r.unlocked
                        ? "bg-primary/20"
                        : "bg-white/5"
                      }`}
                  >
                ...
                <h3
                  className={`text-xs font-bold leading-tight mb-1 ${r.unlocked ? "text-white" : "text-slate-400"
                    }`}
                >
                  {r.title}
                </h3>
                ...
                {!r.unlocked && (
                  <p className="text-[8px] text-slate-600">{t("until", { date: validDate })}</p>
                )}
              </div>
            </div>
          );
