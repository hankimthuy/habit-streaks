...
function GoalCard({ goal, onLog, onDelete, onEdit, loadingGoals }: {
  ...
  const statusLabel = isPerfect ? t("perfect") : ratio >= 0.5 ? t("onTrack") : t("keepGoing");
      ...
      <div className="bg-gradient-to-br from-surface-dark to-black border border-slate-800 rounded-3xl p-5 relative">
        ...
        <div className="flex justify-between items-start mb-4 relative z-10">
          ...
          <div className="flex flex-col items-end">
            ...
            <span
              className={`text-[10px] font-bold ${statusColor} ${isPerfect ? "bg-primary/10" : "bg-accent-green/10"
        ...
        <div className="flex gap-1 h-2 w-full mb-3">
          {Array.from({ length: goal.target_days }).map((_, i) => (
            <div
              key={i}
              className={`flex-1 rounded-full ${i < goal.current_streak ? colors.bar : "bg-slate-700"
                }`}
            />
          ))}
        </div>
          ...
          <div className="flex items-center justify-between relative z-10">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                disabled={isLoading}
                className={`h-8 px-4 rounded-full font-bold text-xs flex items-center gap-1 transition-all ${isLoading
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-4 px-1 mt-2">
        <h2 className="text-xl font-bold">{t("goalStreaks")}</h2>
